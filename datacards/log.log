...
Main options:
  ...
  -M [ --method ] arg (=ProfileLikelihood)
                                        Method to extract upper limit. 
                                        Supported methods are: Asymptotic, 
                                        AsymptoticNew, BayesianSimple, 
                                        BayesianToyMC, ChannelCompatibilityChec
                                        k, FeldmanCousins, GenerateOnly, 
                                        GoodnessOfFit, HybridNew, 
                                        MarkovChainMC, MaxLikelihoodFit, 
                                        MultiDimFit, ProfileLikelihood
  -v [ --verbose ] arg (=1)             Verbosity level (-1 = very quiet; 0 = 
                                        quiet, 1 = verbose, 2+ = debug)
  -h [ --help ]                         Produce help message

Common statistics options:
  -S [ --systematics ] arg (=1)         Add systematic uncertainties
  ...
  --setPhysicsModelParameters arg       Set the values of relevant physics 
                                        model parameters. Give a comma 
                                        separated list of parameter value 
                                        assignments. Example: CV=1.0,CF=1.0
  --setPhysicsModelParameterRanges arg  Set the range of relevant physics model
  ...
  --redefineSignalPOIs arg              Redefines the POIs to be this 
                                        ...
                                        the workspace.
  --freezeNuisances arg                 Set as constant all these nuisance 
                                        parameters.
...
Common input-output options:
  ...
  --floatNuisances arg                  Set to floating these nuisance 
                                        parameters (note freeze will take 
                                        priority over float)
...
Cascade Minimizer options:
  ...
  --cminOldRobustMinimize arg (=1)      Use the old 'robustMinimize' logic in 
  ...
  --cminDefaultMinimizerType arg (=Minuit)
...
Common miscellaneous options:
  --newGenerator arg (=1)               Use new generator code for toys, fixes 
                                        all issues with binned and mixed 
                                        generation (equivalent of --newToyMC 
                                        but affects the top-level toys from 
                                        option '-t' instead of the ones within 
                                        the HybridNew)
...
Asymptotic specific options:
  ...
  --singlePoint arg                     Just compute CLs for the given value of
                                        r
  --minimizerAlgo arg (=Minuit2)        Choice of minimizer used for profiling 
                                        (Minuit vs Minuit2)
  --minimizerTolerance arg (=0.00999999978)
                                        Tolerance for minimizer used for 
                                        profiling
  --minimizerStrategy arg (=0)          Stragegy for minimizer
...
AsymptoticNew specific options:
  --run arg (=both)     What to run: both (default), observed, expected.
  --nPoints arg (=20)   Number of points in scan for CLs
  --qtilde arg (=1)     Allow only non-negative signal strengths (default is 
                        true).
  --scanMin arg (=0)    Minimum value for scan in r.
  --scanMax arg (=20)   Maximum value for scan in r.
...
ChannelCompatibilityCheck specific options:
  --minimizerAlgo arg (=Minuit2)        Choice of minimizer (Minuit vs Minuit2)
  --minimizerTolerance arg (=0.100000001)
                                        Tolerance for minimizer
  --minimizerStrategy arg (=1)          Stragegy for minimizer
  --preFitValue arg (=1)                Value of signal strength pre-fit
  ...
  --stepSize arg (=0.100000001)         Step size for robust fits (multiplier 
                                        of the range)
  --minimizerAlgoForMinos arg (=Minuit2,simplex)
                                        Choice of minimizer (Minuit vs Minuit2)
                                        for profiling in robust fits
  --minimizerStrategyForMinos arg (=0)  Stragegy for minimizer for profiling in
                                        robust fits
  --minimizerToleranceForMinos arg (=9.99999975e-05)
...
GoodnessOfFit specific options:
  --algorithm arg                       Goodness of fit algorithm. Supported 
                                        algorithms are 'saturated', 'KS' and 
                                        'AD'.
  --minimizerAlgo arg (=Minuit2)        Choice of minimizer (Minuit vs Minuit2)
  --minimizerTolerance arg (=9.99999975e-05)
                                        Tolerance for minimizer
  --minimizerStrategy arg (=1)          Stragegy for minimizer
  --fixedSignalStrength arg (=0)        Compute the goodness of fit for a fixed
                                        signal strength. If not specified, it's
                                        left floating
  --plots                               Make plots containing information of 
                                        the computation of the Anderson-Darling
                                        or Kolmogorov-Smirnov test statistic
...
HybridNew specific options:
  ...
  --onlyTestStat                        Just compute test statistics, not 
                                        actual p-values (works only with 
                                        --singlePoint)
  ...
  --generateExternalMeasurements arg (=0)
                                        ...
                                        of the ModelConfig
  --fitNuisances arg (=0)               Fit the nuisances, when not generating 
                                        them.
  ...
  --fork arg (=1)                       Fork to N processes before running the 
                                        toys (set to 0 for debugging)
...
MaxLikelihoodFit specific options:
  --minimizerAlgo arg (=Minuit2)        Choice of minimizer (Minuit vs Minuit2)
  --minimizerTolerance arg (=0.100000001)
                                        Tolerance for minimizer
  --minimizerStrategy arg (=1)          Stragegy for minimizer
  ...
  --minos arg (=poi)                    Compute MINOS errors for: 'none', 
                                        'poi', 'all'
  --out arg (=.)                        Directory to put output in
  --plots                               Make plots
  ...
  --backgroundPdfNames arg (=shapeBkg*) Names of background pdfs in plots 
                                        (separated by ',')
  --saveNormalizations                  Save post-fit normalizations of all 
                                        components of the pdfs
  ...
  --saveOverallShapes                   Save total shapes (and covariance if 
                                        used with saveWithUncertainties) across
                                        all channels
  --saveWithUncertainties               Save also post-fit uncertainties on the
                                        shapes and normalizations (from 
  ...
  --customStartingPoint                 Don't set the signal model parameters 
                                        to zero before the fit
...
MultiDimFit specific options:
  ...
  -P [ --poi ] arg                      Parameters of interest to fit (default 
                                        = all)
