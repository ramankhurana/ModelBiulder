...
do           
	...
	##
        echo "combine higgsCombinePostFit.MultiDimFit.mH120.root --snapshotName MultiDimFit -M Asymptotic  -n randomtest   --bypassFrequentistFit  --freezeNuisanceGroups ^$nuisancename" > nuisance.sh
	...
	##
        ##PERFORM ASYMPTOTIC LIMIT CALCULATION WITH NUISANCE AS ONLY SOURCE
        ##
	echo "combine higgsCombinePreFit.MultiDimFit.mH120.root --snapshotName MultiDimFit -M Asymptotic  -n randomtest   --bypassFrequentistFit   --freezeNuisanceGroups ^$nuisancename" > nuisance.sh
	...
	median_expected_prefit_onlysource=`cat nuisance_${nuisancename}_limit_tmp.txt | grep 'Median for expected limits' | awk '{print $5}'`
        ...
        ##
        echo "combine higgsCombinePreFit.MultiDimFit.mH120.root --snapshotName MultiDimFit -M Asymptotic  -n randomtest   --bypassFrequentistFit --minimizerAlgo Minuit --freezeNuisanceGroups $nuisancename" > nuisance.sh
