...
samples_2hdm=(BarZp-${mzp}-${mchi})
#samples_2hdm=(BarZp-1000-1000)

#echo "med dm twosigdown onesigdown exp onesigup twosigup observed" >> limits_barzp.txt
...
for k in "${samples_2hdm[@]}"; do
    ...
    #sed -i 's/XX-SIGNAL-XX/'${k}'/g' dir_${mzp}_${mchi}/combined.txt
    sed -i 's/XX-SIGNAL-XX/'${k}'/g' combined.txt
    
    #Computing limits
    txtfilename=limits_tmp${mzp}_${mchi}.txt
    #cd dir_${mzp}_${mchi}
    combine -M Asymptotic combined.txt --rAbsAcc 0 --rMax 30 --mass ${mzp}${mchi} | tee ${txtfilename}
    ...
    echo "${mediator} ${dm} ${twosigdown} ${onesigdown} ${exp} ${onesigup} ${twosigup} ${observed}" >> limits_barzp.txt
