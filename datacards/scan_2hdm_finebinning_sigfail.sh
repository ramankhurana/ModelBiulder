...
#samples_2hdm=(ZpA0-3500-300)

echo "med dm twosigdown onesigdown exp onesigup twosigup observed" > limits_2hdm_finebinning_sigfail.txt
...
for k in "${samples_2hdm[@]}"; do
    ...
    cp combined_tmpl_finebinning_sigfail.txt combined.txt
    ...
    echo "${mediator} ${dmmass} ${twosigdown} ${onesigdown} ${exp} ${onesigup} ${twosigup} ${observed}" >> limits_2hdm_finebinning_sigfail.txt
done
