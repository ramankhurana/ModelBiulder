...
import plotConfig_mT as plotConfig
...
def plotPreFitPostFit(region):
  ...
  f_mlfit = TFile(basedir+'/datacards/%s' % sys.argv[1],'READ')
...
#      'qcd',
  ...
  f_prefit = open('prefit_masked_%s.txt' % region, 'w')
...
#      print >> f_prefit, process + ' ' + str(h_prefit[process].GetXaxis().GetBinLowEdge(i)) + ' ' + str(h_prefit[process].GetXaxis().GetBinUpEdge(i)) + ' ' + str(h_prefit[process].GetBinContent(i)*h_prefit[process].GetBinWidth(i)) + ' ' + str(h_prefit[process].GetBinError(i)*h_prefit[process].GetBinWidth(i)) + ' ' + str(h_prefit[process].GetBinError(i)*h_prefit[process].GetBinWidth(i)/(h_prefit[process].GetBinContent(i)*h_prefit[process].GetBinWidth(i)))
  ...
  f_postfit = open('postfit_masked_%s.txt' % region, 'w')
...
#  c.Range(0,0,1,1);
  ...
  dummy.SetMinimum(0.0000005)
  
  if region == "singlemuonw" or region == "singleelectronw":
    dummy.SetMaximum(100000)
    dummy.SetMinimum(0.0000017)
  if region == "singlemuontop" or region == "singleelectrontop":
    dummy.SetMaximum(30000)
    dummy.SetMinimum(0.000001)
  if region == "singlemuontop_fail" or region == "singleelectrontop_fail":
    dummy.SetMaximum(100000)
...
#    legend.AddEntry(h_fcnc,'FCNC M_{V}=0.9 TeV','l')
  ...
  c.SaveAs(plotsDir+"srmasked_monoHbb_"+region+".pdf")
  c.SaveAs(plotsDir+"srmasked_monoHbb_"+region+".png")
  c.SaveAs(plotsDir+"srmasked_monoHbb_"+region+".C")
