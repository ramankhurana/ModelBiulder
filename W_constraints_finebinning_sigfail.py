...
def cmodel(cid,nam,_f,_fOut, out_ws, diag):
  ...
  controlmc_sigfail    = _fin.Get("signal_fail_wjets")  # defines in / out acceptance
  ...
  WScales_sig_fail.Divide(controlmc_sigfail);  _fOut.WriteTObject(WScales_sig_fail)  
  ...
  #dummy for now:
  h_trig_up = targetmc.Clone()
  h_trig_down = targetmc.Clone()
  for b in range(h_trig_up.GetNbinsX()): h_trig_up.SetBinContent(b+1,1.01)
  for b in range(h_trig_down.GetNbinsX()): h_trig_down.SetBinContent(b+1,0.99)
...
#   Channel("signalfailwModel",_wspace,out_ws,cid+'_'+model,WScales_sig_fail),
  ]
  ...
  CRs.append(Channel("signalfailwModel",_wspace,out_ws,cid+'_'+model,WScales_sig_fail))
  ...
  addStatErrs(WScales_sig_fail,CRs[6],'sigw_fail','signalfailwModel')
